...
describe('CustomersService related works', () => {
  function customerFixture(technicianId = 7) {
    return {
      ...
      phone: '13800000000',
      address: null,
      avatarUrl: '/legacy-avatar.jpg',
      clientUser: { nickname: '雅雅', avatarUrl: '/account-avatar.jpg' },
      orders: [],
      revenues: [],
      technician: { id: technicianId, name: 'Luna', phone: '13900000000' },
      workAccesses: [
        {
          orderId: 31,
          canView: true,
  ...
  it('客户详情返回当前客户的可见关联作品与独立权限', async () => {
    const prisma = {
      customer: { findUnique: jest.fn().mockResolvedValue(customerFixture()) },
    };
    ...
    expect(result).toEqual(
      expect.objectContaining({
        accountName: '雅雅',
        avatarUrl: 'http://localhost:3000/account-avatar.jpg',
      }),
    );
    expect(result).not.toHaveProperty('clientUser');
  });
  ...
  it('其他美甲师不能读取客户及其关联作品', async () => {
    ...
    const service = new CustomersService(prisma as never);
    await expect(service.findOneForTechnician(12, 9)).rejects.toBeInstanceOf(
      ForbiddenException,
    );
  });
});
