...
- 定金、尾款和订阅支付统一使用独立支付单资源；订单只保存聚合支付状态。
...
- 小程序调用 `wx.login`，后端使用 `code2Session` 获取 OpenID；AppSecret 只存在后端环境变量中。
- 登录页加载时只调用 `wx.login` 预检测绑定状态并缓存短期结果，不自动建立本地业务登录态；用户明确点击微信登录或切换到手机号登录后才继续对应流程。
- `WechatIdentity` 以 `appId + openId` 唯一，同一微信身份可以分别关联客户和美甲师角色。
- 未绑定身份只返回十分钟有效的 `wechatSessionToken`，不向客户端返回 OpenID 或 `session_key`。
- 用户主动点击 `getPhoneNumber` 后，后端使用动态 `phoneCode` 获取微信验证手机号；新客户仍需美甲师邀请，新美甲师仍需管理员邀请密钥。
- 已有手机号账号在手机号验证成功后绑定微信；已绑定其他微信或其他账号时拒绝覆盖。
- 微信能力未配置、授权被拒绝或接口失败时，继续保留手机号密码和短信找回入口。
- Access Token 失效后由小程序使用对应角色 Refresh Token 单飞续期并重放一次原请求，续期失败才退出登录。
- 验证码按角色和用途隔离，数据库只保存带 Pepper 的哈希、有效期、错误次数及限流窗口。
- 微信 AppID、AppSecret、商户号、APIv3 Key 和商户私钥由超管后台维护；敏感值使用 `SYSTEM_CONFIG_ENCRYPTION_KEY` 经 AES-256-GCM 加密后入库，管理端只返回是否已配置，不回显原文。
- 登录配置通过微信 Access Token 接口在线校验，支付配置使用商户 API 私钥签名请求微信支付平台证书接口校验；修改配置立即使旧校验状态失效。
- `/api/public/capabilities` 只返回微信登录、微信支付是否可用及通用原因。小程序默认隐藏微信能力，只有“管理员启用 + 配置完整 + 在线校验成功”时才展示；能力接口失败时继续隐藏。
...
支付单至少保存：业务支付号、订单或订阅目标、阶段、金额（分）、渠道、状态、幂等键、渠道交易号、创建/支付/关闭/退款时间。资质未完成时渠道返回 `channel_pending`，不能写入 `paid`。未来微信支付适配器只负责预下单、验签回调和退款；支付业务状态仍由统一支付服务维护。
