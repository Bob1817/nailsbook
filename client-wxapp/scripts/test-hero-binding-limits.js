...
assert.deepEqual(summary.previewTechnicians.map(w => w.id), [1,5,4,3,2]);
assert.equal(summary.hiddenActiveCount, 0);
assert.equal(summary.previewSummary, '1 位待确认');
...
(async () => {
  ...
  console.log('Hero满额显式替换、失败保留、重复点击与5位横向预览/待审计数通过');
