...
(async () => {
  ...
  await guest.bookNow();
  assert.match(destination, /^\/pages\/client\/create-order\/index\?/);
  assert.match(destination, /invite=INVITE7/);
  assert.ok(!destination.includes('quickBookingTechId='));
  assert.equal(storage.post_auth_redirect, undefined, '选时间前不触发登录');
  ...
  await guest.bookNow();
  assert.equal(calls.some(call => Array.isArray(call)), false, '进入时间页不提前写绑定');
