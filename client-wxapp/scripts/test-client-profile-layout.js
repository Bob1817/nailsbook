...
assert(template.includes('scroll-x'), '多位美甲师使用横向紧凑预览');
...
assert(template.includes('class="tech-relationship-line"'));
assert(template.includes('item.relationship.lifetimeSpendText'));
...
assert(!styles.includes('min-height:264rpx'), '无备注时预览卡应按内容收缩');
assert(styles.includes('min-height: 44px'));
assert(read('styles/technician-relationship.wxss').includes('.tech-quick-book'));
