  ...
  return (
    <div className="flex h-[100dvh] flex-col bg-[var(--nb-page)]">
      ...
      <div className="shrink-0 flex items-center gap-3 bg-white/95 px-5 py-3.5 backdrop-blur border-b border-[var(--nb-line)]">
        ...
        <h1 className="text-[17px] font-semibold text-[var(--nb-ink)]">店铺管理</h1>
      </div>
      ...
      <div className="px-5 pt-4">
        <div className="rounded-xl bg-[var(--nb-page)] p-4 border border-[var(--nb-line)]">
          <div className="flex items-start gap-3">
            ...
            <div>
              <p className="text-sm text-[var(--nb-ink)] font-medium">到店美甲配置</p>
      ...
      <div className="px-5 py-6 space-y-4">
        {shops.length > 0 ? (
          shops.map((shop, index) => (
            <div
              key={index}
              className="rounded-2xl bg-white p-5 shadow-sm border border-[var(--nb-line)]"
            >
              <div className="flex items-start justify-between gap-3">
                <div className="flex-1 min-w-0">
                  <div className="mb-2 flex items-center gap-2">
                    <span className="text-2xl">🏪</span>
                    <h3 className="text-lg font-semibold text-[var(--nb-ink)]">{shop.name}</h3>
                    <span
                      className={`rounded-full px-2.5 py-1 text-xs font-medium ${
                        shop.enabled ?? true ? 'bg-[var(--nb-page)] text-[var(--nb-secondary)]' : 'bg-[var(--nb-page)] text-[var(--nb-muted)]'
                      }`}
                    >
                  ...
                  <p className="mt-2 text-xs leading-relaxed text-[var(--nb-muted)]">
                    {formatBusinessHours(shop.businessHours)}
                  </p>
                  {shop.doorInfo && (
                    <p className="text-xs text-[var(--nb-muted)] mt-1">{shop.doorInfo}</p>
                  )}
                  {shop.phone && (
                    <p className="text-sm text-[var(--nb-secondary)] mt-2 flex items-center gap-1">
              ...
              <div className="mt-4 flex items-center justify-end gap-2 border-t border-[var(--nb-line)] pt-4">
                <button
                  ...
                  className="min-h-11 rounded-full bg-[var(--nb-page)] px-4 py-2 text-sm font-medium text-[var(--nb-secondary)]"
                >
          ...
          <div className="rounded-2xl bg-white px-6 py-12 text-center shadow-sm border border-[var(--nb-line)]">
            ...
            <p className="text-base font-medium text-[var(--nb-ink)]">暂无店铺</p>
            <p className="mt-2 text-sm text-[var(--nb-muted)]">添加店铺信息，让客户知道您的店铺位置</p>
          </div>
        )}
      </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-[var(--nb-line)] px-5 py-4 safe-area-bottom">
        <div className="mx-auto max-w-md">
          <button
            ...
            className="w-full rounded-full bg-[var(--nb-action)] py-3.5 font-medium text-white shadow-lg"
          >
            + 新增店铺
          </button>
        </div>
