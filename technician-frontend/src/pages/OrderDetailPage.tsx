...
const OrderDetailPage: React.FC<OrderDetailPageProps> = ({ orderIdProp, onClose, isModal }) => {
  ...
  if (isLoading) {
    return (
      <div className="min-h-full bg-[var(--nb-page)] flex items-center justify-center">
  ...
  const content = (
    <div className={isModal ? 'flex flex-col h-full bg-[var(--nb-page)]' : 'min-h-full bg-[var(--nb-page)] pb-28'}>
      {/* Header */}
      <div className={isModal
        ? 'shrink-0 z-10 flex items-center justify-between bg-white/95 px-5 py-3.5 backdrop-blur border-b border-[var(--nb-line)]'
        : 'sticky top-0 z-10 flex items-center justify-between bg-white/95 px-5 py-3.5 backdrop-blur border-b border-[var(--nb-line)]'
      }>
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-[17px] font-semibold text-[var(--nb-ink)]">预约详情</h1>
            <p className="text-[12px] text-[var(--nb-muted)]">{order.orderNo}</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          {!isEditing && (
            <Button
              ...
              className="h-11 rounded-[18px] px-5 text-[14px] shadow-[0_10px_24px_rgba(0,0,0,0.18)]"
            >
              编辑
            </Button>
          )}
      ...
      <div className={isModal ? 'flex-1 min-h-0 overflow-y-auto space-y-5 px-5 py-5' : 'space-y-5 px-5 py-5'}>
        ...
        <div className="rounded-[24px] border border-[var(--nb-line)] bg-white p-5 shadow-[0_8px_24px_rgba(0,0,0,0.04)]">
          <div className="flex items-center justify-between">
            <div>
              <p className="mb-2 text-[12px] text-[var(--nb-muted)]">当前状态</p>
            ...
            {order.serviceType && (
              <div className="text-right">
                <p className="mb-2 text-[12px] text-[var(--nb-muted)]">服务类型</p>
                <span className={`inline-flex items-center px-3 py-1 rounded-full text-sm font-medium ${
                  order.serviceType === 'home' ? 'bg-[var(--nb-page)] text-[var(--nb-secondary)]' : 'bg-[var(--nb-page)] text-[var(--nb-secondary)]'
                }`}>
        ...
        {/* 客户信息 */}
        <div className="rounded-[24px] border border-[var(--nb-line)] bg-white p-5 shadow-[0_8px_24px_rgba(0,0,0,0.04)]">
          <h2 className="mb-4 text-[18px] font-semibold text-[var(--nb-ink)]">客户信息</h2>
          <div className="space-y-3">
            <div className="flex items-center gap-3">
              <div className="flex h-14 w-14 items-center justify-center rounded-full bg-[var(--nb-page)] text-[22px] font-semibold text-[var(--nb-secondary)]">
                {order.customerName.charAt(0)}
              </div>
              <div>
                <p className="text-[16px] font-semibold text-[var(--nb-ink)]">{order.customerName}</p>
                {customer && (
                  <p className="mt-1 text-[14px] text-[var(--nb-secondary)]">{customer.phone}</p>
                )}
              </div>
            </div>
            {customer && customer.tags.length > 0 && (
              <div className="flex flex-wrap gap-2">
                {customer.tags.map((tag, i) => (
                  <span key={i} className="px-2 py-1 bg-[var(--nb-page)] text-[var(--nb-secondary)] rounded text-xs">
                    {tag}
                  </span>
                ))}
              </div>
        ...
        {/* 服务信息 */}
        <div className="rounded-[24px] border border-[var(--nb-line)] bg-white p-5 shadow-[0_8px_24px_rgba(0,0,0,0.04)]">
          <h2 className="mb-4 text-[18px] font-semibold text-[var(--nb-ink)]">服务信息</h2>
          {isEditing ? (
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-[var(--nb-ink)] mb-2">服务类型</label>
                  ...
                  <button
                    ...
                    className={`flex-1 py-3 rounded-xl border text-sm font-medium ${
                      editForm.serviceType === 'home'
                        ? 'border-[var(--nb-ink)] bg-[var(--nb-page)] text-[var(--nb-secondary)]'
                        : 'border-[var(--nb-line)] text-[var(--nb-secondary)]'
                    }`}
                  >
                    上门美甲
                  </button>
              ...
              {editForm.serviceType === 'shop' && (
                <div>
                  <label className="block text-sm font-medium text-[var(--nb-ink)] mb-2">选择店铺</label>
                  <select
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-sm font-medium text-[var(--nb-ink)] mb-2">开始时间</label>
                  <input
                    type="datetime-local"
                ...
                <div>
                  <label className="block text-sm font-medium text-[var(--nb-ink)] mb-2">结束时间</label>
                  <input
                    type="datetime-local"
              ...
              <div>
                <label className="block text-sm font-medium text-[var(--nb-ink)] mb-2">服务地址/店铺</label>
                <p className="px-4 py-3 rounded-xl bg-[var(--nb-page)] text-[var(--nb-secondary)]">
                  {editForm.serviceType === 'home' ? order.address : (editForm.shopId || '请选择店铺')}
                </p>
              </div>

              <div>
                <label className="block text-sm font-medium text-[var(--nb-ink)] mb-2">备注</label>
                <textarea
                  value={editForm.note}
                  onChange={(e) => setEditForm({ ...editForm, note: e.target.value })}
                  placeholder="添加备注信息..."
                  className="w-full px-4 py-3 rounded-xl border border-[var(--nb-line)] focus:border-[var(--nb-ink)] focus:outline-none min-h-[80px]"
                />
              </div>
            </div>
          ) : (
            <div className="space-y-4">
              <div className="flex items-start justify-between gap-4">
                <span className="text-[14px] text-[var(--nb-muted)]">服务内容</span>
                <span className="max-w-[62%] text-right text-[15px] font-medium text-[var(--nb-ink)]">{order.serviceName}</span>
              </div>
              <div className="flex items-start justify-between gap-4">
                <span className="text-[14px] text-[var(--nb-muted)]">预约时间</span>
                <span className="max-w-[62%] text-right text-[15px] font-medium text-[var(--nb-ink)]">
                  {formatDateLabel(order.startTime)} {formatTimeRange(order.startTime, order.endTime)}
                </span>
              </div>
              <div className="flex items-start justify-between gap-4">
                <span className="text-[14px] text-[var(--nb-muted)]">服务时长</span>
                <span className="text-[15px] font-medium text-[var(--nb-ink)]">
                  {getDurationMinutes(order.startTime, order.endTime)} 分钟
                </span>
              </div>
              <div className="flex items-start justify-between gap-4">
                <span className="text-[14px] text-[var(--nb-muted)]">{order.serviceType === 'shop' ? '服务店铺' : '服务地址'}</span>
                <span className="max-w-[62%] text-right text-[15px] font-medium leading-6 text-[var(--nb-ink)]">
                  {order.serviceType === 'shop' ? (order.shopName || '到店服务') : order.address}
                </span>
              </div>
              {order.customDescription && (
                <div className="border-t border-[var(--nb-line)] pt-3">
                  <span className="text-[14px] text-[var(--nb-muted)]">需求描述</span>
                  <p className="mt-2 whitespace-pre-wrap text-[14px] leading-6 text-[var(--nb-ink)]">
                    {order.customDescription}
                  </p>
                </div>
              )}
              {order.customImages && order.customImages.length > 0 && (
                <div className="border-t border-[var(--nb-line)] pt-3">
                  <span className="text-[14px] text-[var(--nb-muted)]">参考图</span>
              ...
              {order.note && (
                <div className="border-t border-[var(--nb-line)] pt-3">
                  <span className="text-[14px] text-[var(--nb-muted)]">备注</span>
                  <p className="mt-2 text-[14px] leading-6 text-[var(--nb-ink)]">{order.note}</p>
                </div>
              )}
            </div>
          )}
        ...
        {/* 价格明细 */}
        <div className="rounded-[24px] border border-[var(--nb-line)] bg-white p-5 shadow-[0_8px_24px_rgba(0,0,0,0.04)]">
          <h2 className="mb-4 text-[18px] font-semibold text-[var(--nb-ink)]">价格明细</h2>
          {isEditing ? (
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-[var(--nb-ink)] mb-2">基础报价</label>
                <input
                  type="number"
              ...
              {editForm.serviceType === 'home' && (
                <div>
                  <label className="block text-sm font-medium text-[var(--nb-ink)] mb-2">上门费用</label>
                  <input
                    type="number"
              ...
              <div>
                <label className="block text-sm font-medium text-[var(--nb-ink)] mb-2">夜间服务费</label>
                <input
                  type="number"
              ...
              <div>
                <label className="block text-sm font-medium text-[var(--nb-ink)] mb-2">节假日服务费</label>
                <input
                  type="number"
              ...
              <div>
                <label className="block text-sm font-medium text-[var(--nb-ink)] mb-2">其他费用</label>
                <input
                  type="number"
              ...
              <div className="pt-4 border-t border-[var(--nb-line)]">
                <div className="flex justify-between items-center mb-3">
                  <span className="font-semibold text-[var(--nb-ink)]">总价</span>
                  <span className="text-xl font-bold text-[var(--nb-secondary)]">{formatMoney(totalPrice)}</span>
                </div>
                <div>
                  <label className="block text-sm font-medium text-[var(--nb-ink)] mb-2">定金金额（线下收取）</label>
                  <input
                    type="number"
            ...
            <div className="space-y-4">
              <div className="flex items-start justify-between gap-4">
                <span className="text-[14px] text-[var(--nb-muted)]">基础报价</span>
                <span className="text-[15px] font-medium text-[var(--nb-ink)]">
              ...
              {order.priceBreakdown?.homeServiceFee ? (
                <div className="flex items-start justify-between gap-4">
                  <span className="text-[14px] text-[var(--nb-muted)]">上门费用</span>
                  <span className="text-[15px] font-medium text-[var(--nb-ink)]">{formatMoney(order.priceBreakdown.homeServiceFee)}</span>
                </div>
              ) : null}
              {order.priceBreakdown?.nightFee ? (
                <div className="flex items-start justify-between gap-4">
                  <span className="text-[14px] text-[var(--nb-muted)]">夜间服务费</span>
                  <span className="text-[15px] font-medium text-[var(--nb-ink)]">{formatMoney(order.priceBreakdown.nightFee)}</span>
                </div>
              ) : null}
              {order.priceBreakdown?.holidayFee ? (
                <div className="flex items-start justify-between gap-4">
                  <span className="text-[14px] text-[var(--nb-muted)]">节假日服务费</span>
                  <span className="text-[15px] font-medium text-[var(--nb-ink)]">{formatMoney(order.priceBreakdown.holidayFee)}</span>
                </div>
              ) : null}
              {order.priceBreakdown?.otherFees ? (
                <div className="flex items-start justify-between gap-4">
                  <span className="text-[14px] text-[var(--nb-muted)]">其他费用</span>
                  <span className="text-[15px] font-medium text-[var(--nb-ink)]">{formatMoney(order.priceBreakdown.otherFees)}</span>
                </div>
              ) : null}
              <div className="flex items-center justify-between border-t border-[var(--nb-line)] pt-4">
                <span className="text-[18px] font-semibold text-[var(--nb-ink)]">总价</span>
                <span className="text-[32px] font-semibold tracking-[-0.03em] text-[var(--nb-secondary)]">{formatMoney(order.price)}</span>
              </div>
              <div className="flex items-start justify-between gap-4">
                <span className="text-[14px] text-[var(--nb-muted)]">定金</span>
                <span className={`text-[15px] font-medium ${order.depositPaid ? 'text-[var(--nb-secondary)]' : 'text-[var(--nb-secondary)]'}`}>
        ...
        {order.serviceItems && order.serviceItems.length > 0 && (
          <div className="rounded-[24px] border border-[var(--nb-line)] bg-white p-5 shadow-[0_8px_24px_rgba(0,0,0,0.04)]">
            <h2 className="mb-4 text-[18px] font-semibold text-[var(--nb-ink)]">服务作品</h2>
            <div className="grid grid-cols-3 gap-3">
              {order.serviceItems.map((item) => (
                <div key={item.id} className="aspect-square rounded-xl bg-[var(--nb-page)] overflow-hidden">
                  {item.images && item.images[0] ? (
                    <img src={item.images[0]} alt={item.name} className="w-full h-full object-cover" />
                  ) : (
                    <div className="w-full h-full flex items-center justify-center text-[var(--nb-muted)] text-xs">
                      {item.name}
                    </div>
                  )}
                </div>
      ...
      {/* 底部操作栏 */}
      <div className={isModal
        ? 'shrink-0 border-t border-[var(--nb-line)] bg-white/95 px-5 py-4 backdrop-blur-xl'
        : 'fixed bottom-0 left-0 right-0 border-t border-[var(--nb-line)] bg-white/95 px-5 py-4 backdrop-blur-xl'
        ...
        {isEditing ? (
          <div className="flex gap-3">
            <button
              ...
              className="flex-1 h-12 rounded-[18px] border border-[var(--nb-line)] bg-white text-[15px] font-medium text-[var(--nb-ink)] active:opacity-80"
            >
              取消
            </button>
            <button
              ...
              className="flex-1 h-12 rounded-[18px] bg-[var(--nb-action)] text-[15px] font-medium text-white shadow-none disabled:opacity-50 active:opacity-80"
            >
          ...
          order.status === 'expired' ? (
            <button
              ...
              className="w-full h-12 rounded-[18px] bg-[var(--nb-action)] text-[15px] font-medium text-white shadow-none active:opacity-80"
            >
              重新发起预约
            </button>
          ) : (
          ...
          >
            <div className="shrink-0 px-6 pt-6 pb-4 flex items-center justify-between">
              <div>
                ...
                <p className="mt-1 text-sm text-[var(--nb-secondary)]">仅需重新选择预约时间，其余信息将沿用原预约</p>
              </div>
              <button
                type="button"
                onClick={() => setShowReinit(false)}
                disabled={isReinitiating}
                className="flex h-9 w-9 items-center justify-center rounded-full bg-[var(--nb-page)] disabled:opacity-50"
              >
            ...
            <div className="flex-1 overflow-y-auto px-6 space-y-4 pb-4">
              <div className="rounded-3xl bg-[var(--nb-page)] p-4">
                ...
                <input
                  ...
                  className="w-full rounded-2xl bg-white px-4 py-3 text-[var(--nb-ink)] outline-none ring-1 ring-[var(--nb-line)] focus:ring-[var(--nb-control)]/30"
                />
              </div>
              <div className="rounded-3xl bg-[var(--nb-page)] p-4">
                  ...
                  {reinitTimeSlots.map((time) => (
                    <button
                      ...
                      className={`rounded-2xl py-2.5 text-sm font-medium transition ${
                        reinitTime === time
                          ? 'bg-[var(--nb-action)] text-white shadow'
            ...
            <div className="shrink-0 px-6 pb-[max(1.5rem,env(safe-area-inset-bottom)+1rem)] pt-2">
              <button
                ...
                disabled={isReinitiating}
                className="w-full rounded-full bg-[var(--nb-action)] py-4 font-medium text-white shadow disabled:opacity-50"
              >
  ...
  return (
    <div className="fixed inset-0 z-[150] flex items-end justify-center bg-black/40 backdrop-blur-sm" onClick={onClose}>
      <div
        className="w-full max-w-md h-[92vh] rounded-t-3xl bg-white overflow-hidden shadow-[0_-20px_50px_rgba(0,0,0,0.18)]"
        onClick={(e) => e.stopPropagation()}
