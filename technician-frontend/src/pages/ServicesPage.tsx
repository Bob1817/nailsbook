...
const categoryColors: Record<ServiceCategory, { bg: string; text: string; border: string }> = {
  basic_care: { bg: 'bg-[var(--nb-page)]', text: 'text-[var(--nb-secondary)]', border: 'border-[var(--nb-line)]' },
  color_style: { bg: 'bg-[var(--nb-page)]', text: 'text-[var(--nb-secondary)]', border: 'border-[var(--nb-line)]' },
  extension_reinforcement: { bg: 'bg-[var(--nb-page)]', text: 'text-[var(--nb-secondary)]', border: 'border-[var(--nb-line)]' },
  removal: { bg: 'bg-[var(--nb-page)]', text: 'text-[var(--nb-secondary)]', border: 'border-[var(--nb-line)]' },
};
    ...
    <div className="flex h-[100dvh] flex-col bg-[var(--nb-page)]">
      ...
      <div className="shrink-0 flex items-center justify-between bg-white/95 px-5 py-3.5 backdrop-blur border-b border-[var(--nb-line)]">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-[17px] font-semibold text-[var(--nb-ink)]">服务管理</h1>
        </div>
        <div className="flex items-center gap-2">
          <button
            onClick={handleAddNew}
            className="flex h-11 w-11 items-center justify-center rounded-full bg-[var(--nb-action)] text-white shadow-md"
          >
      ...
      <div className="flex-1 overflow-y-auto px-5 py-6 space-y-6 pb-24">
        {services.length === 0 ? (
          <div className="rounded-2xl bg-white px-6 py-12 text-center shadow-sm border border-[var(--nb-line)]">
            <div className="mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-full bg-[var(--nb-page)]">
              <span className="text-3xl">💅</span>
            </div>
            <p className="text-base font-medium text-[var(--nb-ink)]">暂无服务</p>
            <p className="mt-2 text-sm text-[var(--nb-muted)]">点击右上角添加您的第一个服务</p>
          </div>
        ) : (
            ...
            return (
              <div key={category}>
                <div className={`flex items-center gap-2 mb-3 px-1`}>
                  ...
                  <span className="text-xs text-[var(--nb-muted)]">({categoryServices.length})</span>
                </div>
                  ...
                  {categoryServices.map((service) => (
                    <div
                      ...
                      className={`rounded-2xl bg-white p-4 shadow-sm border ${
                        service.isActive ? 'border-[var(--nb-line)]' : 'border-[var(--nb-line)] opacity-60'
                      }`}
                    >
                      <div className="flex items-start justify-between gap-3">
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2 mb-1">
                            <h3 className={`text-base font-semibold ${service.isActive ? 'text-[var(--nb-ink)]' : 'text-[var(--nb-secondary)]'}`}>
                              {service.name}
                            </h3>
                            {!service.isActive && (
                              <span className="text-[10px] px-1.5 py-0.5 bg-[var(--nb-page)] text-[var(--nb-secondary)] rounded">
                                已禁用
                              </span>
                            )}
                          </div>
                      ...
                      {/* Actions */}
                      <div className="mt-4 flex items-center justify-end gap-2 border-t border-[var(--nb-line)] pt-3">
                        <button
                          onClick={() => handleEdit(service)}
                          className="rounded-full bg-[var(--nb-page)] px-4 py-2 text-sm font-medium text-[var(--nb-secondary)]"
                        >
                          编辑
                        </button>
                        <button
            ...
            <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-6 space-y-5">
              <div>
                <label className="block text-sm font-medium text-[var(--nb-ink)] mb-2">
                  服务名称 <span className="text-[var(--nb-secondary)]">*</span>
                </label>
                <input
                  type="text"
                  value={formData.name}
              ...
              <div>
                <label className="block text-sm font-medium text-[var(--nb-ink)] mb-2">
                  服务分类 <span className="text-[var(--nb-secondary)]">*</span>
                </label>
                <div className="grid grid-cols-2 gap-3">
                  {(Object.keys(SERVICE_CATEGORIES) as ServiceCategory[]).map((category) => (
                    ...
                    >
                      ...
                      <span className="text-xs text-[var(--nb-secondary)] mt-1">
