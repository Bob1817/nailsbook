...
class _ClientProfileScreenState extends State<ClientProfileScreen> {
  ...
  Widget build(BuildContext context) {
    if (_loading) return const Scaffold(body: Center(child: CircularProgressIndicator(color: DT.primary)));
    ...
    return Scaffold(
      ...
      body: Stack(
        children: [
          ...
          const Center(child: Text('v1.0.0', style: TextStyle(color: DT.textMuted, fontSize: 12))),
            ],
          ),
              ...
              Container(
                ...
                child: CircleAvatar(
                  ...
                  child: (avatarUrl == null || avatarUrl.isEmpty)
                      ? Text(nickname.isNotEmpty ? nickname.substring(0, 1) : '?', style: const TextStyle(fontSize: 24, color: DT.textPrimary))
                      : null,
                ),
              ),
        ...
        children: [
          CircleAvatar(
            ...
            child: (tech.avatarUrl == null || tech.avatarUrl!.isEmpty)
                ? Text(tech.name.substring(0, 1), style: const TextStyle(color: DT.primary))
                : null,
          ),
          ...
          Expanded(
            child: Column(
              ...
              children: [
                Row(
                  children: [
                    Flexible(child: Text(tech.name, overflow: TextOverflow.ellipsis, style: const TextStyle(fontSize: 15, fontWeight: FontWeight.w600, color: DT.textPrimary))),
                    if (tech.isDefault == true) ...[
                      const SizedBox(width: 6),
                      Container(
                        padding: const EdgeInsets.symmetric(horizontal: 6, vertical: 2),
                        decoration: BoxDecoration(color: DT.primarySoft, borderRadius: BorderRadius.circular(6)),
                        child: const Text('默认', style: TextStyle(color: DT.primaryDark, fontSize: 10, fontWeight: FontWeight.w600)),
                      ),
                    ],
                  ],
                ),
                if (tech.city != null && tech.city!.isNotEmpty) ...[
                  ...
                  Text(tech.city!, style: const TextStyle(fontSize: 12, color: DT.textSecondary)),
                ],
              ],
            ),
          ),
  ...
  Widget _sectionTitle(String title) {
    return Padding(
      ...
      child: Text(title, style: const TextStyle(fontSize: 14, fontWeight: FontWeight.w600, color: DT.textSecondary)),
    );
  }
  ...
  Widget _menuCard(List<_MenuItem> items) {
    return Container(
      ...
      decoration: BoxDecoration(color: DT.surface, borderRadius: BorderRadius.circular(16), border: Border.all(color: DT.border)),
      ...
      child: Column(
        children: List.generate(items.length, (i) {
          ...
          return Column(
            children: [
              if (i > 0) const Divider(height: 1, indent: 60, color: DT.divider),
              Material(
                ...
                child: ListTile(
                  leading: Container(
                    ...
                    child: Icon(item.icon, color: DT.primary, size: 19),
                  ),
                  title: Text(item.label, style: const TextStyle(fontSize: 15, color: DT.textPrimary)),
                  trailing: const Icon(Icons.chevron_right_rounded, size: 20, color: DT.textQuaternary),
                  onTap: item.onTap,
                ),
              ),
            ],
              ...
              child: Container(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    const Text('绑定美甲师', style: TextStyle(fontSize: 20, fontWeight: FontWeight.w600, color: DT.textPrimary)),
                    const SizedBox(height: 6),
                    const Text('输入美甲师提供的邀请码完成绑定', style: TextStyle(fontSize: 13, color: DT.textMuted)),
                    ...
                    CupertinoTextField(
                      ...
                      style: const TextStyle(fontSize: 16, color: DT.textPrimary, letterSpacing: 1.5),
                      placeholderStyle: const TextStyle(fontSize: 16, color: DT.textMuted, letterSpacing: 0),
                      decoration: BoxDecoration(
                        color: DT.surfaceAlt,
                        ...
                        child: Row(children: [
                          ...
                          Expanded(
                            child: Column(
                              crossAxisAlignment: CrossAxisAlignment.start,
                              children: [
                                Text(found!.name, style: const TextStyle(fontSize: 15, fontWeight: FontWeight.w600, color: DT.textPrimary)),
                                if (found!.city != null) ...[
                                  const SizedBox(height: 2),
                                  Text(found!.city!, style: const TextStyle(fontSize: 12, color: DT.textMuted)),
                                ],
                              ],
                            ),
                          ),
                          const Icon(CupertinoIcons.checkmark_circle_fill, color: DT.primary, size: 22),
                        ]),
                      ),
                    ],
                    ...
                    SizedBox(
                      ...
                      child: CupertinoButton(
                        ...
                        borderRadius: BorderRadius.circular(999),
                        color: DT.primary,
                        disabledColor: DT.primary.withOpacity(0.35),
                        ...
                        child: binding
                            ...
                            : const Text('确认绑定', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w600, color: Colors.white)),
                      ),
                    ),
                  ],
                ),
