...
const MainLayout: React.FC = () => {
  ...
  if (loading) {
    return (
      <div className="h-[100dvh] flex items-center justify-center bg-[var(--nb-page)]">
  ...
  const tabs = [
    {
      ...
      label: '首页',
      icon: (active: boolean) => (
        <svg className={`w-6 h-6 ${active ? 'text-[var(--nb-ink)]' : 'text-[var(--nb-muted)]'}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
  ...
  return (
    <div className="h-[100dvh] bg-[var(--nb-page)] flex flex-col max-w-md mx-auto overflow-hidden">
      {/* Main Content */}
      ...
      {/* Bottom Tab Bar */}
      {showTabBar && (
        <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-[var(--nb-line)] safe-area-bottom z-50">
          <div className="max-w-md mx-auto flex items-center justify-around py-2">
            {tabs.map((tab) => {
              ...
              return (
                ...
                >
                  {tab.icon(active)}
                  <span className={`text-xs ${active ? 'text-[var(--nb-ink)] font-medium' : 'text-[var(--nb-muted)]'}`}>
                    {tab.label}
                  </span>
                </button>
              );
